...
/datum/preferences/New(client/C, datum/db_query/Q) // Process our query
	...
	if(istype(C))
		...
		if(!successful_load)
			to_chat(C, "<span class='narsie'>Не удалось загрузить ваши настройки. Пожалуйста, немедленно сообщите хосту сервера.</span>")
...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	dat += "<a href='?_src_=prefs;preference=tab;tab=[TAB_GAME]' [current_tab == TAB_GAME ? "class='linkOn'" : ""]>Настройки игры</a>"
	dat += "<a href='?_src_=prefs;preference=tab;tab=[TAB_GEAR]' [current_tab == TAB_GEAR ? "class='linkOn'" : ""]>Снаряжение</a>"
	dat += "<a href='?_src_=prefs;preference=tab;tab=[TAB_KEYS]' [current_tab == TAB_KEYS ? "class='linkOn'" : ""]>Горячие клавиши</a>"
	dat += "</center>"
	dat += "<HR>"
	...
	switch(current_tab)
		if(TAB_CHAR) // Character Settings
			...
			dat += "<a href='?_src_=prefs;preference=name;task=random'>(Случайное)</a>"
			dat += "<a href='?_src_=prefs;preference=name'><span class='[active_character.be_random_name ? "good" : "bad"]'>(Всегда Случайное)</span></a><br>"
			dat += "</td><td width='405px' height='25px' valign='left'>"
			dat += "<center>"
			dat += "Слот <b>[default_slot][active_character.from_db ? "" : " (пустой)"]</b><br>"
			...
			dat += "<h2>Личность</h2>"
			dat += "<b>Пол:</b> <a href='?_src_=prefs;preference=gender'>[active_character.gender == MALE ? "Мужской" : (active_character.gender == FEMALE ? "Женский" : "Отсутствует")]</a>"
			dat += "<br>"
			dat += "<b>Возраст:</b> <a href='?_src_=prefs;preference=age;task=input'>[active_character.age]</a><br>"
			dat += "<b>Тело:</b> <a href='?_src_=prefs;preference=all;task=random'>(&reg;)</a><br>"
			dat += "<b>Раса:</b> <a href='?_src_=prefs;preference=species;task=input'>[active_character.species]</a><br>"
			if(active_character.species == "Vox") // Purge these bastards
				dat += "<b>N2 баллон:</b> <a href='?_src_=prefs;preference=speciesprefs;task=input'>[active_character.speciesprefs ? "Large N2 Tank" : "Specialized N2 Tank"]</a><br>"
			if(active_character.species == "Grey")
				dat += "<b>Wingdings:</b> Настройте в инвалидности<br>"
				dat += "<b>Переводчик голоса:</b> <a href ='?_src_=prefs;preference=speciesprefs;task=input'>[active_character.speciesprefs ? "Да" : "Нет"]</a><br>"
			dat += "<b>Второй язык:</b> <a href='?_src_=prefs;preference=language;task=input'>[active_character.language]</a><br>"
			if(S.autohiss_basic_map)
				dat += "<b>Авто-акцент:</b> <a href='?_src_=prefs;preference=autohiss_mode;task=input'>[active_character.autohiss_mode == AUTOHISS_FULL ? "Полный" : (active_character.autohiss_mode == AUTOHISS_BASIC ? "Частичный" : "Отключен")]</a><br>"
			dat += "<b>Группа крови:</b> <a href='?_src_=prefs;preference=b_type;task=input'>[active_character.b_type]</a><br>"
			if(S.bodyflags & (HAS_SKIN_TONE|HAS_ICON_SKIN_TONE))
				dat += "<b>Тон кожи:</b> <a href='?_src_=prefs;preference=s_tone;task=input'>[S.bodyflags & HAS_ICON_SKIN_TONE ? "[active_character.s_tone]" : "[-active_character.s_tone + 35]/220"]</a><br>"
			dat += "<b>Инвалидность:</b> <a href='?_src_=prefs;preference=disabilities'>\[Настроить\]</a><br>"
			dat += "<b>Отношение к Нанотрейзен:</b> <a href ='?_src_=prefs;preference=nt_relation;task=input'>[active_character.nanotrasen_relation]</a><br>"
			dat += "<a href='byond://?_src_=prefs;preference=flavor_text;task=input'>Указать описание персонажа</a><br>"
			...
			if(S.bodyflags & HAS_HEAD_ACCESSORY) //Species that have head accessories.
				...
				dat += "<a href='?_src_=prefs;preference=headaccessory;task=input'>Цвет</a> [color_square(active_character.hacc_colour)]<br>"
			...
			if(S.bodyflags & HAS_HEAD_MARKINGS) //Species with head markings.
				...
				dat += "<a href='?_src_=prefs;preference=m_head_colour;task=input'>Цвет</a> [color_square(active_character.m_colours["head"])]<br>"
			if(S.bodyflags & HAS_BODY_MARKINGS) //Species with body markings/tattoos.
				...
				dat += "<a href='?_src_=prefs;preference=m_body_colour;task=input'>Цвет</a> [color_square(active_character.m_colours["body"])]<br>"
			if(S.bodyflags & HAS_TAIL_MARKINGS) //Species with tail markings.
				...
				dat += "<a href='?_src_=prefs;preference=m_tail_colour;task=input'>Цвет</a> [color_square(active_character.m_colours["tail"])]<br>"
			if(!(S.bodyflags & BALD))
				dat += "<b>Волосы:</b> "
				dat += "<a href='?_src_=prefs;preference=h_style;task=input'>[active_character.h_style]</a>"
				dat += "<a href='?_src_=prefs;preference=hair;task=input'>Цвет</a> [color_square(active_character.h_colour)]"
				...
				if(temp_hair_style && temp_hair_style.secondary_theme && !temp_hair_style.no_sec_colour)
					dat += " <a href='?_src_=prefs;preference=secondary_hair;task=input'>Цвет #2</a> [color_square(active_character.h_sec_colour)]"
				// Hair gradient
				dat += "<br>"
				dat += "- <b>Градиент:</b>"
				dat += " <a href='?_src_=prefs;preference=h_grad_style;task=input'>[active_character.h_grad_style]</a>"
				dat += " <a href='?_src_=prefs;preference=h_grad_colour;task=input'>Цвет</a> [color_square(active_character.h_grad_colour)]"
				dat += " <a href='?_src_=prefs;preference=h_grad_alpha;task=input'>[active_character.h_grad_alpha]</a>"
				dat += "<br>"
				dat += "- <b>Смещение градиента:</b> <a href='?_src_=prefs;preference=h_grad_offset;task=input'>[active_character.h_grad_offset_x],[active_character.h_grad_offset_y]</a>"
				dat += "<br>"

				dat += "<b>Лицевая Растительность:</b> "
				dat += "<a href='?_src_=prefs;preference=f_style;task=input'>[active_character.f_style ? "[active_character.f_style]" : "Бритый"]</a>"
				dat += "<a href='?_src_=prefs;preference=facial;task=input'>Цвет</a> [color_square(active_character.f_colour)]"
				...
				if(temp_facial_hair_style && temp_facial_hair_style.secondary_theme && !temp_facial_hair_style.no_sec_colour)
					dat += " <a href='?_src_=prefs;preference=secondary_facial;task=input'>Цвет #2</a> [color_square(active_character.f_sec_colour)]"
				dat += "<br>"


			if(!(S.bodyflags & ALL_RPARTS))
				dat += "<b>Глаза:</b> "
				dat += "<a href='?_src_=prefs;preference=eyes;task=input'>Цвет</a> [color_square(active_character.e_colour)]<br>"
			...
			if((S.bodyflags & HAS_SKIN_COLOR) || ((S.bodyflags & HAS_BODYACC_COLOR) && GLOB.body_accessory_by_species[active_character.species]) || check_rights(R_ADMIN, 0, user)) //admins can always fuck with this, because they are admins
				dat += "<b>Цвет Тела:</b> "
				dat += "<a href='?_src_=prefs;preference=skin;task=input'>Цвет</a> [color_square(active_character.s_colour)]<br>"

			if(GLOB.body_accessory_by_species[active_character.species] || check_rights(R_ADMIN, 0, user))
				dat += "<b>Тело:</b> "
				dat += "<a href='?_src_=prefs;preference=body_accessory;task=input'>[active_character.body_accessory ? "[active_character.body_accessory]" : "Ничего"]</a><br>"

			dat += "</td><td width='405px' height='200px' valign='top'>"
			dat += "<h2>Выбор профессии</h2>"
			dat += "<a href='?_src_=prefs;preference=job;task=menu'>Установить Предпочтения Профессии</a><br>"
			if(jobban_isbanned(user, ROLEBAN_RECORDS))
				dat += "<b>Вам запрещено использовать записи персонажей.</b><br>"
			else
				dat += "<a href=\"byond://?_src_=prefs;preference=records;record=1\">Записи Персонажа</a><br>"
			...
			dat += "<b>Конечности и части тела:</b> <a href='?_src_=prefs;preference=limbs;task=input'>Изменить</a><br>"
			if(active_character.species != "Slime People" && active_character.species != "Machine")
				dat += "<b>Внутренние Органы:</b> <a href='?_src_=prefs;preference=organs;task=input'>Изменить</a><br>"
			...
			if(S.clothing_flags & HAS_UNDERWEAR)
				dat += "<b>Нижнее белье:</b> <a href ='?_src_=prefs;preference=underwear;task=input'>[active_character.underwear]</a><BR>"
			if(S.clothing_flags & HAS_UNDERSHIRT)
				dat += "<b>Майка:</b> <a href ='?_src_=prefs;preference=undershirt;task=input'>[active_character.undershirt]</a><BR>"
			if(S.clothing_flags & HAS_SOCKS)
				dat += "<b>Носки:</b> <a href ='?_src_=prefs;preference=socks;task=input'>[active_character.socks]</a><BR>"
			dat += "<b>Тип рюкзака:</b> <a href ='?_src_=prefs;preference=bag;task=input'>[active_character.backbag]</a><br>"
