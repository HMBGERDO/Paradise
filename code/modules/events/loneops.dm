...
/datum/event/loneops/proc/make_ops()
	...
	if(candidates.len >= 1)
		...
		H.equipOutfit(/datum/outfit/admin/syndicate/operative/lone)
		var/player_tc = max(20, round(temp.total_tc / temp.agents_possible)) // Lone Operative would not get less then 20TC
